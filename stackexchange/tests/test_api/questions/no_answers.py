...
class QuestionNoAnswerTests(BaseQuestionTestCase):
    ...
    def test_sort_by_activity(self):
        ...
        """
        response = self.client.get(reverse('api-question-no-answers'), data={'sort': 'activity', 'order': 'asc'})
        self.assert_sorted(response, 'last_activity_date')

        response = self.client.get(reverse('api-question-no-answers'), data={'sort': 'activity', 'order': 'desc'})
    ...
    def test_sort_by_creation_date(self):
        ...
        """
        response = self.client.get(reverse('api-question-no-answers'), data={'sort': 'creation', 'order': 'asc'})
        self.assert_sorted(response, 'creation_date')

        response = self.client.get(reverse('api-question-no-answers'), data={'sort': 'creation', 'order': 'desc'})
        self.assert_sorted(response, 'creation_date', reverse=True)

    def test_sort_by_votes(self):
        """Test the question no answers endpoint sorted by votes.
        """
        response = self.client.get(reverse('api-question-no-answers'), data={'sort': 'votes', 'order': 'asc'})
        self.assert_sorted(response, 'score')

        response = self.client.get(reverse('api-question-no-answers'), data={'sort': 'votes', 'order': 'desc'})
