...
class UserListTests(base.BaseTestCase):
    ...
    def test_sort_by_reputation(self):
        ...
        for order in enums.OrderingDirection:
            response = self.client.get(reverse('api-user-list'), data={'sort': 'reputation', 'order': order.value})
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assert_user_response(response)
    ...
    def test_sort_by_creation(self):
        ...
        """
        for order in enums.OrderingDirection:
            response = self.client.get(reverse('api-user-list'), data={'sort': 'creation', 'order': order.value})
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assert_user_response(response)
    ...
    def test_sort_by_name(self):
        ...
        """
        for order in enums.OrderingDirection:
            response = self.client.get(reverse('api-user-list'), data={'sort': 'name', 'order': order.value})
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assert_user_response(response)
    ...
    def test_sort_by_modified(self):
        ...
        """
        for order in enums.OrderingDirection:
            response = self.client.get(reverse('api-user-list'), data={'sort': 'modified', 'order': order.value})
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assert_user_response(response)
    ...
    def test_date_range(self):
        ...
        response = self.client.get(
            reverse('api-user-list'), data={'fromdate': from_date.isoformat(), 'todate': to_date.isoformat()}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
