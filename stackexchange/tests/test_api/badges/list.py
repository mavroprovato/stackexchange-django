...
class BadgeListTests(BadgeWithAwardCountTestCase):
    ...
    def test_sort_by_rank(self):
        ...
        """
        response = self.client.get(reverse('api-badge-list'), data={'sort': 'rank', 'order': 'asc'})
        self.assert_sorted(response, 'rank', transform=lambda x: enums.BadgeClass[x.upper()].value)

        response = self.client.get(reverse('api-badge-list'), data={'sort': 'rank', 'order': 'desc'})
    ...
    def test_sort_by_name(self):
        ...
        """
        response = self.client.get(reverse('api-badge-list'), data={'sort': 'name', 'order': 'asc'})
        self.assert_sorted(response, 'name', case_insensitive=True)

        response = self.client.get(reverse('api-badge-list'), data={'sort': 'name', 'order': 'desc'})
    ...
    def test_sort_by_type(self):
        ...
        """
        response = self.client.get(reverse('api-badge-list'), data={'sort': 'type', 'order': 'asc'})
        self.assert_sorted(response, 'badge_type', transform=lambda x: enums.BadgeType[x.upper()].value)

        response = self.client.get(reverse('api-badge-list'), data={'sort': 'type', 'order': 'desc'})
