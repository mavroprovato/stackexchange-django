...
class BadgeNamedTests(base.BaseTestCase):
    ...
    def test_sort_by_rank(self):
        ...
        for order in enums.OrderingDirection:
            response = self.client.get(reverse('api-badge-named'), data={'sort': 'rank', 'order': order.value})
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assert_badge_with_award_count_response(response)
    ...
    def test_sort_by_name(self):
        ...
        """
        for order in enums.OrderingDirection:
            response = self.client.get(reverse('api-badge-named'), data={'sort': 'name', 'order': order.value})
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assert_badge_with_award_count_response(response)
