...
class PostCommentsTests(base.BaseTestCase):
    ...
    def test_sort_by_creation_date(self):
        """Test the post comment retrieve endpoint sorted by creation date.
        """
    ...
    def test_sort_by_votes(self):
        """Test the post comment retrieve endpoint sorted by votes.
        """
        for order in enums.OrderingDirection:
            comments = random.sample(list(models.PostComment.objects.all()), 3)
            response = self.client.get(
    ...
    def test_range_by_creation_date(self):
        """Test the post comment retrieve endpoint range by creation date.
        """
        comments = random.sample(list(models.PostComment.objects.all()), 3)
    ...
    def test_range_by_votes(self):
        """Test the post comment retrieve endpoint range by votes.
        """
        comments = random.sample(list(models.PostComment.objects.all()), 3)
    ...
    def test_date_range(self):
        """Test the post comment retrieve endpoint date range.
        """
        comments = random.sample(list(models.PostComment.objects.all()), 3)
        from_date, to_date = self.generate_random_date_range()
        response = self.client.get(
            reverse('api-post-comments', kwargs={'pk': ';'.join(str(comment.pk) for comment in comments)}), data={
